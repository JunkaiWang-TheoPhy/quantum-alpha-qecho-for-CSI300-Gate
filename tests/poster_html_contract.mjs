...
for (const claim of ["让<span class=\"market\">市场路径</span>", "THE MARKET REMEMBERS", "0.8327", "7/9", "4096", "0.53%", "不是端到端五步预测", "OPEN-SYSTEM MEMORY", "SLD-QFI", "FAIR LEDGER"]) {
...
assert.equal((html.match(/class="journey-label/g) || []).length, 3, "poster must contain three journey labels");
assert.doesNotMatch(html, /Q-SWITCH/, "Q-SWITCH must be removed from the poster");
assert.ok(html.indexOf('class="team"') < html.indexOf("<header>"), "team strip must appear above the title");
assert.ok(html.includes("assets/poster-qr.png"), "title must include the live-poster QR code");
