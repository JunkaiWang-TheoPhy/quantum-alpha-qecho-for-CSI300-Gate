...
assert.equal((html.match(/<small>指导老师<\/small>/g) || []).length, 1, "visible advisor role must appear exactly once");
assert.equal((html.match(/<strong>罗迪<\/strong>/g) || []).length, 1, "visible advisor name must appear exactly once");
assert.doesNotMatch(html, /class="advisor-note"|指导老师 · 罗迪/, "framed advisor credit must be removed");
assert.ok(html.includes('class="portrait-card advisor-card"'), "advisor must appear as the fifth portrait card");
assert.ok(html.includes("assets/portrait-luo-di-v3.png"), "advisor portrait must use the backed-up yellow artwork");
assert.equal((html.match(/Q-ECHO-V/g) || []).length, 1, "Q-ECHO-V may appear only in the supplied document title");
assert.doesNotMatch(html, /旧版网页|evidence\/report\.html|evidence-first-landing|清华大学物理系|Advisor|ADVISOR|4\s*\+\s*1|4 位参与者|4位参与者/);
...
assert.equal((html.match(/class="portrait-card/g) || []).length, 5, "gallery must contain four participant cards and one advisor card");
